...
async function isLicenseValid(doc: any) {
  ...
  if (expDate < currentDate || doc.use >= 1) {
    console.log("Clé expirée");
    return { valid: false, expirationDate: expirationDate };
  }

  return { valid: true, expirationDate: expirationDate };
}
...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Si la licence est valide, incrémenter le compteur d'utilisation
    if (valid) {
      const { error } = await supabaseClient
        .from("users")
        .update({ use: (doc.use || 0) + 1 })
        .eq("id", licenseKey);

      if (error) {
        console.error(
          "Erreur lors de la mise à jour du compteur d'utilisation:",
          error
        );
      }
    }
