...
class StatsScreenState extends ViewStateAbs {
  ...
  double get maxY => dataToShow.values.isNotEmpty
      ? dataToShow.values
              .reduce((int max, int value) => max > value ? max : value)
              .toDouble() +
          yInterval
      : 5;
  ...
  /// Total
  double get total =>
      orders.fold(0, (double sum, Order order) => sum + order.price.toInt());
  ...
  Map<String, double> get totalByShop => orders.fold<Map<String, double>>(
        <String, double>{},
        (Map<String, double> map, Order order) => map
          ..update(
            order.shopName,
            (double value) => value + order.price,
  ...
  Map<DateTime, int> get dailyRevenue {
    ...
    final Map<DateTime, int> amountsMap = <DateTime, int>{};

    // Ajouter les montants pour les dates dans la plage
    for (final Order order in orders) {
      final DateTime orderDate = order.startDate.getDateWithoutTime();
      ...
      amountsMap.update(
        orderDate,
        (int value) => (value + order.price).toInt(),
  ...
  Map<DateTime, int> get ordersQuantityByDate {
    ...
    final Map<DateTime, int> ordersMap = <DateTime, int>{};
